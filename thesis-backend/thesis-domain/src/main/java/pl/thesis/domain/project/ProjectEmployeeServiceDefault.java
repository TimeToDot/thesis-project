...
import pl.thesis.domain.employee.CalendarProjectService;
...
@AllArgsConstructor
@Service
public class ProjectEmployeeServiceDefault implements ProjectEmployeeService {
    ...
    public void setProjectApprovalsEmployee(ProjectApprovalTasksPayloadDTO payloadDTO, PagingSettings settings){
        ...
        setTasksToApprove(payloadDTO, settings, project, accountProject);

        if (!payloadDTO.tasks().isEmpty()) {
            setTasksToReject(payloadDTO);
        }
    }
    ...
    public CalendarDTO getProjectEmployeeCalendar(Long projectEmployeeId, Long projectId, Date date) {
        ...
        return calendarProjectService.getEmployeeCalendarForProject(projectAccount.getAccount().getId(), projectId, date);
    }
    ...
    private void setTasksToReject(ProjectApprovalTasksPayloadDTO payloadDTO) {
    ...
    private void setTasksToApprove(ProjectApprovalTasksPayloadDTO payloadDTO, PagingSettings settings, Project project, AccountProject accountProject) {
    ...
    private ProjectApprovalDTO getProjectApprovalDTO(Project project, AccountProject accountProject) {
        ...
        var calendar = calendarProjectService.getCalendarTaskDTOList(taskList);
