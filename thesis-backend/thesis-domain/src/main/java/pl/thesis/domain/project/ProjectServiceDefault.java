...
public class ProjectServiceDefault implements ProjectService {
    ...
    public Long updateProject(ProjectUpdatePayloadDTO payloadDTO){
        ...
        var status = payloadDTO.active() ? ProjectType.ACTIVE : ProjectType.INACTIVE;
        
        boolean isPermissionChangeNeeded = project.getOwner().getId().compareTo(newOwner.getId()) != 0;

        var projectId = updateProjectDetails(payloadDTO, project, projectDetails, newOwner, status);
    ...
    public ProjectCalendarDTO getProjectCalendar(Long id, Long projectId, Date date) {
        ...
        var employeeId = getEmployeeIdByProjectEmployee(id);
        var calendarDTO = calendarProjectService.getEmployeeCalendarForProject(employeeId, projectId, date);
    ...
    private Long updateProjectDetails(ProjectUpdatePayloadDTO payloadDTO, Project project, ProjectDetails projectDetails, Account newOwner, ProjectType status) {
